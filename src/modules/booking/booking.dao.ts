...
import {
  UpdateBookingRecord,
  createBookingInputRecord,
  passengerRecordPayload,
  seatRecordPayload,
  updateStatusBookingPayload,
...
export const createBookingSeatsRecord = async (
  seatRecordPayload: seatRecordPayload,
  tx: Prisma.TransactionClient,
) => {
...
export const createPassengerRecord = async (
  passengerPayload: passengerRecordPayload,
  tx: Prisma.TransactionClient,
) => {
...
export const upateSeatStatus = async (
  seatId: string,
  updateStatusBookingPayload: updateStatusBookingPayload,
  tx: Prisma.TransactionClient,
) => {
...
export const getBookingSeatsById = async (bookingId: string, tx: Prisma.TransactionClient) => {
  return await tx.booking_seats.findMany({
    where: {
      bookingId,
    },
  });
...
export const cancleBookingById = async (
  bookingId: string,
  updatePayload: UpdateBookingRecord,
  tx: Prisma.TransactionClient,
) => {
...
export const releaseExpiredLocks = async (tx: Prisma.TransactionClient) => {
  return await tx.seat_inventory.updateMany({
    where: {
